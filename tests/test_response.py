...
from src.utils import extract_tool_calls, format_messages_string
...
if "src.eval.email_dataset" in sys.modules:
    importlib.reload(sys.modules["src.eval.email_dataset"])
from src.eval.email_dataset import email_inputs, email_names, response_criteria_list, triage_outputs_list, expected_tool_calls
...
def set_agent_module(agent_module_name):
    ...
    if f"src.{AGENT_MODULE}" in sys.modules:
        importlib.reload(sys.modules[f"src.{AGENT_MODULE}"])
    
    agent_module = importlib.import_module(f"src.{AGENT_MODULE}")
    return AGENT_MODULE
